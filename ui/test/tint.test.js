// Picking a colour out of box art.
//
// The canvas half needs a real graphics stack, which the test environment does
// not have. The half that decides *which* colour, given the pixels, is plain
...
// should read as red, not as very dark grey. A flat average gets that wrong,
...
describe("choosing a cover's colour", () => {
  test("a small bright mark beats a large dark background", () => {
    // The shape of most box art: a dark frame around a colourful logo.
  ...
  test("a grey cover gets no colour rather than a made-up one", () => {
  ...
  test("transparent padding around a cover is ignored", () => {
    ...
    // them would drag every colour towards whatever the blank pixels hold.
  ...
  test("an empty or fully transparent image has no colour", () => {
