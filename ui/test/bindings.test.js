...
describe("the settings tabs", () => {
  ...
  /// One control for the whole look. It was two dropdowns whose only sensible
  /// settings were a matching pair, and before that the glass tint alone.
  test("one theme control drives the whole window", async () => {
    const { paneHtml } = await import(join(uiDir, "js", "settings-panes.js"));
    const pane = new JSDOM(`<body>${paneHtml("appearance")}</body>`).window.document;

    assert.ok(pane.querySelector(".scheme-preset"), "no theme control");
    ...
    // The things that are not part of a theme survive.
